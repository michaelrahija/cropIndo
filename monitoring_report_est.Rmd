...
for (i in 1:length(ids)){
  ...
  test2 <- get_map(location = c(lon = mean(hh$lon), lat = mean(hh$lat)), zoom = 17) 
  test3 <- ggmap(test2)
  ...
  test3 + 
    geom_polygon(aes(x=lon, y=lat), fill='grey', size=.6,color='red', data=poly, alpha=0)+
  geom_point(data = hh, aes(x = lon, y = lat, fill = "red", alpha = 0.8), size = 4, shape = 21, color = "red") +
   guides(fill=FALSE, alpha=FALSE, size=FALSE) +
    labs(x = "longitude", y = "latitude", main = "Map of Interviews") 
}
##LINK THE HH_COOR DATA W/ THE POLYGON!
#IDEALLY A TABLE WITH COLUMN : ID, TYPE - c("Polygon","HH"), LAT,LONG
